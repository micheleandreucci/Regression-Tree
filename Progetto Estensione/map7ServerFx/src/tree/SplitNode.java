...
public abstract class SplitNode extends Node implements Comparable<SplitNode>, Serializable {
	...
	/**
	 * inner class, classe che colleziona informazioni descrittive dallo split
	 */
	...
	/**
	 * Oggetto Attribute che modella l'attributo indipendente sul quale lo split è
	 * generato.
	 */
	 ...
	 */
	List<SplitInfo> mapSplit = new ArrayList<SplitInfo>(); // array per memorizzare gli split candidati
	/**
	...
	SplitNode(Data trainingSet, int beginExampleIndex, int endExampleIndex, Attribute attribute)
		...
		for (int i = 0; i < mapSplit.size(); i++) {
			...
			localVariance = new LeafNode(trainingSet, mapSplit.get(i).getBeginindex(), mapSplit.get(i).getEndIndex())
					.getVariance();
	 ...
	 * un oggetto String. La stringa restituita è necessaria per la predizione di
	 * nuovi esempi.
	 * 
	...
	public int compareTo(SplitNode o) {
		if (o.splitVariance == splitVariance)
			return 0;
		else if (o.splitVariance > splitVariance)
			return -1;
		else
			return 1;
	}
